...
calibration.plot <- function(matched.patients=NULL, g=5,
  ...
  build.plot <- ggplot2::ggplot(data=matched.patients, ggplot2::aes(x= .data$matched.tau.hat),
                ...
                ggplot2::scale_y_continuous(labels=round(seq(from=limits$ymin, to=limits$ymax, length.out=5), 1),
                                                       breaks=round(seq(from=limits$ymin, to=limits$ymax, length.out=5), 1),
                                                       limits=c(limits$ymin, limits$ymax))+
                ggplot2::scale_x_continuous(labels=round(seq(from=limits$xmin, to=limits$xmax, length.out=5), 1),
                                                       breaks=round(seq(from=limits$xmin, to=limits$xmax, length.out=5), 1),
