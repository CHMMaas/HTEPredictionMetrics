...
calibration.plot <- function(matched.patients=NULL, g=5,
  ...
  build.plot <- ggplot2::ggplot(data=matched.patients, ggplot2::aes(x= .data$matched.tau.hat),
    ...
    ggplot2::scale_y_continuous(labels=round(seq(from=min(limits$ymin, -1), to=max(limits$ymax, 1), length.out=5), 2),
                                breaks=round(seq(from=min(limits$ymin, -1), to=max(limits$ymax, 1), length.out=5), 2),
                                limits=c(limits$ymin, limits$ymax))+
    ggplot2::scale_x_continuous(labels=round(seq(from=min(limits$xmin, 0), to=max(limits$xmax, 1), length.out=5), 2),
                                breaks=round(seq(from=min(limits$xmin, 0), to=max(limits$xmax, 1), length.out=5), 2),
